...
# PhidgetMonitor(Channel number, Sensor Name, Topic Name, Channel Mode)
    monitor_0 = PhidgetMonitor(7, 'MaxBotix','sonar','ANALOG')
    monitor_0.setup()
    monitor_2 = PhidgetMonitor(0, 'Proximity Sensor','proximity1','DIGITAL')
    monitor_2.setup()
    monitor_3 = PhidgetMonitor(1, 'Proximity Sensor','proximity2','DIGITAL')
    monitor_3.setup()
    monitor_4 = PhidgetMonitor(2, 'Proximity Sensor','proximity3','DIGITAL')
    monitor_4.setup()
    monitor_5 = PhidgetMonitor(3, 'Proximity Sensor','proximity4','DIGITAL')
    monitor_5.setup()
    monitor_6 = PhidgetMonitor(4, 'Proximity Sensor','proximity5','DIGITAL')
    monitor_6.setup()
    rospy.spin()
